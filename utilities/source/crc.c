...
//CRC16/MODBUS 高位字节值表
const static uint8_t __g_crc16_modbus_table_hi[] =
{
...
//CRC16/MODBUS 低位字节值表
const static uint8_t __g_crc16_modbus_table_lo[] =
{
...
//CRC32/MPEG-2 校验表，生成多项式 0x04c11db7
const static uint32_t __g_crc32_mpeg2_table[] =
{
...
uint16_t crc16_modbus_fast (uint8_t *p_data, uint32_t length)
{
  ...
  {
    idx = crc_lo ^ *p_data++;
    crc_lo = crc_hi ^ __g_crc16_modbus_table_lo[idx];
    crc_hi = __g_crc16_modbus_table_hi[idx];
  }
...
uint32_t crc32_mpeg2_fast (uint32_t initial, void *p_data, uint32_t length)
{
  ...
  for (i = 0; i < length; i++)
  {
    crc = (crc << 8) ^ __g_crc32_mpeg2_table[((crc >> 24) ^ *p_byte++) & 0xFF];
  }
